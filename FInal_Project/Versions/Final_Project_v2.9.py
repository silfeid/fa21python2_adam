...
def get_start_date():
    ...
    if start_month > 0 and start_month < 13:
        ...
        print('Must be integer 1-12. Try again.')
        start_month = integer_checker('Select starting month: ')
...
def comparison_plotter():
    ...
    all_stations_df.drop(all_stations_df.iloc[:, 0], axis=1, inplace=True)
    
    df_columns = list(all_stations_df.columns)
    modify_me = df_columns[0]
    fixed_me = modify_me.rstrip('_y')
    
    
    all_stations_df.rename(columns={all_stations_df.iloc[:,0]:fixed_me}, inplace=True)
