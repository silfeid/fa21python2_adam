...
def choose_char():
    ...
    if char_choice in valid_char_choices:
        with open('fa21python2_adam\\Icon_Project\\charchoice.txt', 'w') as char_doc:
...
def grab_char():
    ...
    with open('fa21python2_adam\\Icon_Project\\charchoice.txt', 'r') as char_doc:
        char = char_doc.read()
    return char
...
def write_csv(icon_list):
    ...
    if 'tempfile' in filepath_text:
        ...
        filepath_text = filepath_text[:-4] + "-tempfile" + filepath_text[-4:]
...
def save_icon():
    ...
    save_over_yes = ['Y', 'y']
    save_name = input('Enter the name for the icon file to be saved (.csv) - file extension not required.  File will be saved at fa21python2_adam\\IconProject\\')
    save_name = 'fa21python2_adam\\Icon_Project\\' + save_name + '.csv'
...
def init_read_in():
    ...
    if invalid_count > 0:
        ...
        with open('fa21python2_adam\\Icon_Project\\rowlength.txt', 'w') as rowlength:
            rowlength.write(str(row_count))
            rowlength.close()

def grab_row_count():
    
    with open('fa21python2_adam\\Icon_Project\\rowlength.txt', 'r') as rowlength:
...
def solicit_filepath():
    ...
    filepath_text = input(
        'Type the name of a CSV file (with extension) in the Icon_Project Folder to feed your own csv file into the icon transformer, otherwise hit enter to proceed with the default icon: ')

    if filepath_text == '':
        filepath_text = 'fa21python2_adam\\Icon_Project\\Icon_Design.csv'

    else:
        filepath_text = 'fa21python2_adam\\Icon_Project\\'+filepath_text
